    ...
    function ($, Marionette) {
        ...
        var DialogView = Marionette.LayoutView.extend({
            ...
            initialize: function(options) {
                ...
                // TODO: Backbone could load this data automatically without this ajax
                var that = this;
                ...
                this.contentTypeList.collection.on("change:isActive", function(model) {
				   var id = model.get("id");
				   if (id != "markdown" && id != "snippets") {
					   id = id + "Diagram";
				   }
                   that.ui.abspath.val("/" + id);
                   that.ui.abspath.trigger("keyup");
                });
            },
            //
            // 1. If "Create" button is pressed then pre-condition if fine
            // 2. If name "checked" then we could pre-save content in the dataProvider ("GitHub" for example)
            // 3. An initial ("dafualt") content for the given type should be requested from the registered content handlers (from the framework)
            // 4. If no dataPath selected then content should be opened without dataProvider and required save AS dialog on close
            //
